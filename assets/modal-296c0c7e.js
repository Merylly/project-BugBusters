import{a as y,i as B,P as $}from"./vendor-7ae4b28f.js";(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))a(t);new MutationObserver(t=>{for(const o of t)if(o.type==="childList")for(const i of o.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function n(t){const o={};return t.integrity&&(o.integrity=t.integrity),t.referrerPolicy&&(o.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?o.credentials="include":t.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(t){if(t.ep)return;t.ep=!0;const o=n(t);fetch(t.href,o)}})();const v={openModalBtn:document.querySelector(".js-open-menu"),closeModalBtn:document.querySelector(".js-close-menu"),modal:document.querySelector(".js-menu-container")};v.openModalBtn.addEventListener("click",k);v.closeModalBtn.addEventListener("click",k);function k(){v.modal.classList.toggle("is-open")}function L({filter:e,page:s=1,limit:n=12,type:a,...t}){return y.get(`https://energyflow.b.goit.study/api/${a}`,{params:{...t,filter:e,page:s,limit:n}})}const h="dateNow",x="quoteDay",E=document.getElementById("description"),S=document.getElementById("author-quote");function I(){const e=new Date,s=`${e.getDate()}.${e.getMonth()+1}.${e.getFullYear()}`,n=localStorage.getItem(h);if(n===s){const a=JSON.parse(localStorage.getItem(x));if(a){const{author:t,quote:o}=a;E.textContent=o,S.textContent=t}else b();return}(!n||n!==s)&&(localStorage.setItem(h,s),b())}function b(){L({type:"quote"}).then(({data:e})=>{const{author:s,quote:n}=e,a={author:s,quote:n};localStorage.setItem(x,JSON.stringify(a)),E.textContent=n,S.textContent=s}).catch(e=>{console.error("Error fetching quote:",e)})}I();window.ResizeObserver=B;function A(e){window.innerWidth<768?pageSize=8:pageSize=12;const s=document.querySelector(".tui-pagination");return new $(s,{totalItems:pageSize*e,itemsPerPage:pageSize,visiblePages:3,centerAlign:!0})}const P=document.querySelectorAll(".switch-item"),M=document.querySelector(".bp-list"),O=document.querySelector(".exercises-page");let c=8,d=1,f;function T(e,s){let n;return function(...t){const o=()=>{clearTimeout(n),e(...t)};clearTimeout(n),n=setTimeout(o,s)}}function q(){window.innerWidth<1440?c=8:c=9}async function D(e){try{return(await L({...e,type:"exercises"})).data}catch(s){throw new Error("Failed to fetch data from API: "+s.message)}}async function u(){try{const e=document.querySelector(".switch-item.is-active");if(!e){console.error("No active container found");return}if(!e.textContent.trim().toLowerCase()){console.error("Query is undefined");return}const n=new URLSearchParams(window.location.search);let a=n.get("bodypart"),t=n.get("muscles"),o=n.get("equipment");if(n.size){const l=await D({bodypart:a,muscles:t,equipment:o}),m=Math.ceil(l.results.length/c);N(l.results.slice((d-1)*c,d*c)),l.results.length>c&&(f&&f.destroy(),f=A(O,m,d,c,U))}const i=new URLSearchParams(window.location.search);window.history.replaceState(null,null,window.location.pathname)}catch(e){console.error("Error fetching and rendering data:",e)}}const w=document.querySelector(".exercises-list");w.addEventListener("click",function(e){const s=e.target.closest(".exercises-item");if(s){e.preventDefault();const n=document.querySelector(".bp-list");n&&n.classList.remove("visually-hidden"),w.classList.add("visually-hidden");const a=new URLSearchParams(window.location.search),t=s.getAttribute("data-filter"),o=s.getAttribute("data-name");a.set(t.toLowerCase(),o.toLowerCase());const i=`${window.location.pathname}?${a.toString()}`;window.history.replaceState(null,null,i),u()}});async function N(e){try{if(!e||e.length===0){console.log("No exercise data to render");return}console.log("Rendering exercise cards with data:",e);let s="";e.forEach(a=>{s+=z(a)}),M.innerHTML=s,document.querySelectorAll(".exercise-card").forEach(a=>{a.addEventListener("click",t=>handleExerciseCardClick(a,t))})}catch(s){console.error("Error rendering exercise cards:",s)}}function z(e){return`
    ...
    </li>`}function r(e){return/^[a-zA-Z]/.test(e)?e.charAt(0).toUpperCase()+e.slice(1):e}function F(){P.forEach(e=>e.classList.remove("is-active")),this.classList.add("is-active"),d=1,u()}function U(e){d=e,u()}P.forEach(e=>{e.addEventListener("click",F)});window.addEventListener("load",()=>{q(),u(),document.querySelectorAll(".exercises-item").forEach(s=>{s.addEventListener("click",n=>handleExerciseCardClick(n))})});window.addEventListener("resize",T(()=>{const e=c;q(),e!==c&&u()},250));const R=document.querySelector(".bp-list"),p=document.querySelector(".backdrop");R.addEventListener("click",K);async function K(e){if(e.target.nodeName!=="BUTTON")return;p.classList.remove("is-hidden");const n=e.target.closest("li").dataset.id;try{const t=await y.get(`https://energyflow.b.goit.study/api/exercises/${n}`);_(t.data);const i=document.querySelector(".stars-wraper").children,l=Math.round(t.data.rating);Q(i,l)}catch(t){console.log(t)}document.querySelector(".modal-close-btn").addEventListener("click",g),window.addEventListener("keydown",t=>{t.code==="Escape"&&g()}),p.addEventListener("click",t=>{t.currentTarget===t.target&&g()})}function g(){p.classList.add("is-hidden"),window.removeEventListener("keydown",e=>{e.code==="Escape"&&g()})}function _({gifUrl:e,name:s,rating:n,target:a,bodyPart:t,equipment:o,burnedCalories:i,popularity:l,description:m}){const C=`<div class="modal">
...
--><span class="block-info-static-value">/3 min</span>
    ...
    `;p.innerHTML=C}function Q(e,s){Array.from(e).forEach((a,t)=>{t<s&&a.classList.remove("dimmed-star")})}export{A as c};
//# sourceMappingURL=modal-296c0c7e.js.map
